...
test('persistent service reloads records, keeps history, and writes audit entries', () => {
  const { directory, url } = createDatabaseUrl();
  try {
    const service = createPersistentEducationPlatformService({ databaseUrl: url });
    ...
    const reloaded = createPersistentEducationPlatformService({ databaseUrl: url });
